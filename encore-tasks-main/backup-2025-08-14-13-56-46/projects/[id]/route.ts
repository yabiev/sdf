...
export async function GET(
  ...
  try {
    ...
    const hasAccess = await databaseAdapter.hasProjectAccess(Number(userId), Number(projectId));
...
export async function PUT(
  ...
  try {
    ...
    const existingProject = await databaseAdapter.getProjectById(Number(projectId));
    if (!existingProject) {
      return NextResponse.json(
        { error: 'Проект не найден' },
        { status: 404 }
    ...
    // Обновление проекта
    const success = await databaseAdapter.updateProject(Number(projectId), updateProjectData);
    ...
    const updatedProject = await databaseAdapter.getProjectById(Number(projectId));
...
export async function DELETE(
  ...
  try {
    ...
    // Удаление проекта
    const success = await databaseAdapter.deleteProject(Number(projectId));
