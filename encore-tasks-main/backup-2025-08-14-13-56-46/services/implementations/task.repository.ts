...
export class TaskRepository implements ITaskRepository {
  ...
  async getMaxPosition(columnId: ColumnId): Promise<number> {
    try {
      const query = 'SELECT MAX(position) as maxPosition FROM tasks WHERE column_id = ? AND is_archived = 0';
  ...
  async existsByTitle(title: string, boardId: BoardId, excludeId?: TaskId): Promise<boolean> {
    try {
      let query = 'SELECT COUNT(*) as count FROM tasks WHERE title = ? AND board_id = ? AND is_archived = 0';
  ...
  async countByBoard(boardId: BoardId): Promise<number> {
    try {
      const query = 'SELECT COUNT(*) as count FROM tasks WHERE board_id = ? AND is_archived = 0';
  ...
  async countByColumn(columnId: ColumnId): Promise<number> {
    try {
      const query = 'SELECT COUNT(*) as count FROM tasks WHERE column_id = ? AND is_archived = 0';
      const result = await this.databaseAdapter.query(query, [columnId]);
  ...
  async countByStatus(boardId: BoardId, status: TaskStatus): Promise<number> {
    try {
      const query = 'SELECT COUNT(*) as count FROM tasks WHERE board_id = ? AND status = ? AND is_archived = 0';
  ...
  async countByPriority(boardId: BoardId, priority: TaskPriority): Promise<number> {
    try {
      const query = 'SELECT COUNT(*) as count FROM tasks WHERE board_id = ? AND priority = ? AND is_archived = 0';
