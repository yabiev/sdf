...
export class ColumnRepository implements IColumnRepository {
  ...
  async create(columnData: CreateColumnDto & { createdBy: UserId }): Promise<Column> {
    try {
      ...
      const query = `
        INSERT INTO columns (
          id, title, board_id, color, position, wip_limit, 
          is_collapsed, settings, created_by, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
  ...
  async update(id: ColumnId, columnData: UpdateColumnDto, updatedBy: UserId): Promise<Column | null> {
    try {
      ...
      if (columnData.wipLimit !== undefined) {
        ...
        params.push(columnData.wipLimit);
      }
  ...
  async getAverageTaskCompletionTime(columnId: ColumnId): Promise<number | null> {
    try {
      const query = `
        ...
        FROM tasks 
        WHERE column_id = ? AND completed_at IS NOT NULL AND is_archived = 0
      `;
