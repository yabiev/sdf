...
export async function POST(request: NextRequest) {
  try {
    ...
    // Проверяем доступ к проекту и права на создание задач
    const taskPermissionService = new TaskPermissionService(dbAdapter);
    const canCreate = await taskPermissionService.canUserCreateTask(Number(user.id), Number(board.projectId), user.role);
    
    if (!canCreate) {
      return NextResponse.json({ error: 'Нет прав на создание задач в этом проекте' }, { status: 403 });
    }
